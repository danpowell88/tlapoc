...
- `backlog/` — the build backlog as [Backlog.md](https://github.com/MrLesk/Backlog.md) tasks (`TLA-*`): 15 epics (parent tasks) with their stories as subtasks, plus the sprint plan in `backlog/docs/`. **Manage via the `backlog` CLI — never hand-edit task files** (keeps frontmatter consistent).
- `backlog.html` — static backlog viewer (epic/story tree, MVP/later filter, board). Reads `backlog/manifest.json`, regenerated by `scripts/backlog_manifest.py` (also run in the Pages Action).
