...
- REQ-MED-11 ★: **Product catalogue & multi-product stock** (ADR-0021/0014) — products are **typed** (toxin/filler/skin) with their own **unit** (units vs syringes/mL) and **par level**; on-hand/usage/wastage/expiry aggregate **per product+unit** (no single "units on hand"); usage history is per product. A capability-gated **catalogue admin** can **add/remove products** and set the **S4 flag** (the single classification that drives rewards eligibility and the public booking-page naming policy everywhere).
...
- REQ-NOTIF-4 *(withdrawn 2026-06-20)*: ~~advertising-compliance linter~~ — **removed from scope.** The platform does **not** lint, check or help produce advertising material; advertising is the clinic's responsibility, handled in its external marketing tools (Mailchimp, Meta Business Suite). The only public product surface — the booking page — uses generic service names + withheld S4 prices by **configuration** (REQ-NOTIF-12), not an automated linter. (See C9.)
...
- ★ **C9 — Advertising compliance (clinic-owned; *not* platform-enforced):** complying with the TGA/AHPRA restrictions on advertising S4 cosmetic injectables — no direct/indirect S4 references (brands, nicknames, blurred logos, generic "anti-wrinkle"/"dermal filler" terms), no price promotions, no banned hashtags (#botox), no promotional before/after, syringe, treatment-animation or facial-mapping imagery, no influencer testimonials, no under-18 targeting, mandatory warnings where permitted — is the **clinic's responsibility**, handled in its **own external marketing tools** (e.g. Mailchimp, Meta Business Suite). **The platform provides no advertising linter and no ad-material production tooling** *(scope cut, 2026-06-20; REQ-NOTIF-4 withdrawn)*. Its only public surface is the **public booking page** (REQ-NOTIF-12), which by **configuration/clinic policy** uses generic service names and withholds S4 prices — a manual setting, not an automated linter. (TGA + AHPRA — context in [market research §4](01-market-research.md).)
...
### GAP-5 — Advertising: newer prohibited content & reach (context for C9)
The live rules tightened: **facial-mapping / treatment-animation imagery** is promotional (June 2025);
...
*Product stance (2026-06-20 scope cut):* the platform provides **no advertising linter and no ad-material
production tooling** — advertising compliance is the clinic's responsibility in its external marketing tools.
The only product surface is the public booking page (generic names + withheld S4 prices by configuration).
C9 is reframed accordingly and **REQ-NOTIF-4 is withdrawn**.
...
| Marketing: omnichannel inbox (IG/FB/SMS/email), categorise, client-link, suggested replies | REQ-NOTIF-6 ➕, ADR-0018/0019 | 🔬 ⏭ (SMS/email in v1) |
...
>
> **Scope adjustments (rev 4.1, 2026-06-20):** (1) **Finances move to Xero & integrations** — the app keeps only **pricing / what-if + high-level reporting**; in-app commission pay-run, supplier POs/AP, refund/dispute management and BAS/GST are **dropped** (ADR-0027 revised; REQ-RPT-6 narrowed; REQ-MED-14, REQ-PAY-7 reclassified external). (2) **Advertising tooling removed** — the advertising linter, newsletter builder & social scheduler are all out of scope; email campaigns, social posting and advertising compliance live in the clinic's existing tools (Mailchimp, Meta Business Suite). The public booking page uses generic names + withheld S4 prices by **configuration**, not a linter (ADR-0034 **withdrawn**; REQ-NOTIF-4/10/11 withdrawn). (3) **Reviews** gain **acknowledge / flag / auto-detect follow-up** for negative reviews & complaints (ADR-0032 extended; REQ-NOTIF-8 strengthened).
...
- **Comms/growth — `NOTIF`:** REQ-NOTIF-8 ★ **reviews/reputation** (request-all, no gating; reply; staff guidance not to publicly repost an S4 result) · REQ-NOTIF-9 ★ **lead/prospect CRM** over the inbox · ~~REQ-NOTIF-10 email/newsletter builder~~ & ~~REQ-NOTIF-11 social scheduler~~ **withdrawn** (advertising handled in the clinic's external tools; no advertising linter) · REQ-NOTIF-12 **public booking widget + SEO** (generic names, S4 price withheld — by configuration) · REQ-MEMB-10 **referral/affiliate depth** (non-S4 credit only). (→ ADR-0032/0033, ~~0034 withdrawn~~; C9/C23)
...
| **Growth** area *(rev 4.1)*: leads CRM; reviews with **acknowledge / flag / auto-detect follow-up** (+ staff guidance not to repost S4 results) — newsletter, social & advertising linter **removed** (advertising handled in Mailchimp/Meta) | REQ-NOTIF-8/9 ➕, REQ-MEMB-10 ➕, ADR-0032/0033 (ADR-0034 withdrawn) | ✅ ➕ |
