...
Each environment has its own Azure Key Vault (provisioned by IAC). The API and pipelines read secrets from the vault at runtime/deploy via managed identity — no secret is in source control, ever. A secret-scanning check in CI fails the build if a secret is committed, so the rule is enforced, not trusted.
...
- [ ] **Add the secret-scanning gate and per-environment config to CI/CD**
  Enforce 'no committed secrets' and select the right config/secrets per deploy.
  - A secret-scanning check in CI that fails the build on a committed secret (coordinated with SEC-BASE's scan suite).
