...
- [ ] **Read-model / projection: compliance metrics + expiry/retention watch**
  Behaviour: project ComplianceMetric values (consent coverage, cooling-off adherence, stock-discrepancy from PRD-04 reconciliation, reg-expiry from credentials) and the ExpiryWatch/RetentionDue lists over the read-models + AuditEvent. Requirements: compute consult-before-script as 100%-by-construction from the absence of unlinked-script audit events; each list carries its basis (AHPRA expiry date / the C18 retention rule); RegisterExport rows are immutable and audited; eventual consistency acceptable.
