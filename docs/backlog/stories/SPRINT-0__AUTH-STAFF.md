...
- [ ] **Configure Entra ID app registration and complete staff SSO + MFA on web and provider app**
  ...
  - Both surfaces complete interactive SSO (single sign-on) + MFA against the clinic's Entra tenant.
...
- [ ] **Document the staff auth setup, claims contract & SaaS-federation path**
  Write the staff-identity reference so API, RBAC (role-based access control) and the SaaS (software-as-a-service) roadmap can rely on it.
