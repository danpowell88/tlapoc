...
- [ ] **Under-18 flag propagation from booking**
  On create, derive under_18 from DOB and stamp it on the Appointment so PRD-03 COOLING-OFF/GATING can enforce the 7-day cooling-off (the mandatory wait before a cosmetic procedure can proceed) + payment block. No cooling-off logic here — only the durable flag + a domain event (a fact emitted when something happens in the system) so downstream gates can react. Guardian/contact capture is deferred to PRD-03 consent.
