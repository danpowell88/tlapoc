...
- [ ] **Server-side consult gate on charting-open (S4 only)**
  When the charting context for an Appointment is requested, evaluate the gate: if service.schedule==S4 (Schedule 4 prescription-only medicine) and Appointment.consult_id is null, refuse to return the editable charting context and return a structured 'missing: consult (the assessment appointment where suitability is established and an S4 prescription can be written)' reason instead. Enforce in the domain/API layer (ADR-0008 (an architecture decision record)), independent of the UI. Add Appointment.consult_id (FK Consult, nullable). Clearing happens when PRD-04 records the consult and sets consult_id.
- [ ] **Gate status on Today/Schedule board + blocked-action banner**
  Surface the per-visit gate state on the Today board and Schedule (chip: consult/consent/screening status). On the Charting screen render the calm blocked-action banner from the structured reason: what's missing, the fix link (record consult (the assessment appointment where suitability is established and an S4 prescription can be written)), and who can resolve it (NP/prescriber (a practitioner authorised to write a prescription)). Reuse the same banner component PRD-03 GATING uses.
- [ ] **Gate-decision audit events**
  Every gate evaluation that BLOCKS (and the later clear) writes an AuditEvent (actor, appointment, reason, timestamp) to the append-only stream (ADR-0010) so the moat is demonstrable to a regulator. Emit a domain event (a fact emitted when something happens in the system) when consult_id is set so the board/banner update live.
