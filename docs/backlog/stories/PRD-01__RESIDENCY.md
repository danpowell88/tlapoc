...
Surfaces to clients as the 'your data stays in Australia' trust note (client app privacy) and to staff as the sub-processor register in Settings. The register is the artefact an auditor asks for under APP 8.
...
- [ ] **Residency verification + APP-8 sub-processor gate**
  Verify (and assert at runtime, tying to the Sprint-0 IaC region policy) that PII/PHI storage + compute resolve to Australia East. Model SubProcessor (region, purpose, data categories, app8_assessment_ref, consent_ref, status) and enforce the gate: any integration sending PII to a non-AU sub-processor is blocked unless an APP-8 assessment AND a consent record both exist, returning a clear reason. This is the check PRD-10 integrations call before any PII leaves.
...
- [ ] **Sub-processor register UI (Settings) + client trust note**
  Build the admin sub-processor register in Settings: a row per integration with region, data categories, APP-8 assessment reference, consent reference and active/blocked status, with the block reason visible. Surface the client-facing 'your data stays in Australia' trust note on the client app privacy screen. Capability-gate the register to owner/configure.
