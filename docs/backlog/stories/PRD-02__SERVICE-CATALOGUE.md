...
- [ ] **Capability-gated catalogue admin + audit**
  Admin endpoints to add/edit/archive a service, gated to prescriber (a practitioner authorised to write a prescription)/owner capability; every change writes an AuditEvent. Validate that S4 (Schedule 4 prescription-only medicine) services cannot have reward/discount controls enabled. Archive (not hard-delete) to preserve history on past bookings.
