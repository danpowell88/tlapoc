...
- [ ] **Treatment-type toggle (toxin / non-S4 skin)**
  Behaviour: a toggle (Anti-wrinkle (toxin) / Skin treatment) selects which template renders and shows/hides the toxin-only map+lot steps vs the non-S4 skin note (prototype setChartType/applyChartType). Requirements: switching swaps the rendered fields and the pre-treatment chips (skin has no consult requirement); the toxin path requires product/lot and prescription, the non-S4 skin path has neither. The skin-note fields themselves are built in PRD-05/MAPPING's skin-note task.
- [ ] **Configurable, versioned NoteTemplate engine + snippet admin**
  Behaviour: the toxin note is a per-tenant NoteTemplate — structured fields (indication, areas, technique notes, aftercare), a free-text area and a curated library of reusable phrases/snippets — with a clinic-side builder/admin and template versioning. Requirements: NoteValue binds captured content to the template version used so a finalised note always renders against the schema it was authored with; the same generic engine is what later modalities (filler, energy, weight-loss) extend, even though v1 ships only the toxin template + the non-S4 skin variant. tenant_id + Row-Level Security (RLS, the per-tenant database isolation) throughout.
