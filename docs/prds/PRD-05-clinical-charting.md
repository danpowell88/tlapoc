...
> **▸ Prototype alignment (rev 2, 2026-06-19).** Charting now **opens with product & batch (lot) selection** (step 1 — the vial being drawn from, restricted to **in-date / ARTG / in-custody** stock) **before** mapping points; charted units **deduct from that selected lot** on finalise (REQ-CLIN-2, REQ-MED-4). Injection mapping (**tap-to-add + drag**) and **before/after drag-compare** are validated. Adds **REQ-CLIN-7 treatment plans & protocols** (multi-session plans + applyable protocol templates feeding recall) and a **charting overview / "in-room now"** entry point. **"Auto-detect" injection points is advisory + human-confirmed only and deferred to Phase 2** (ADR-0020, 🔬 — *no AI in v1*; v1 ships manual mapping). Charting is now a **guided flow** — **pre-treatment review** (safety + last-treatment + consult/Rx surfaced), **treatment-type-aware** (toxin map vs a non-S4 **skin note**) and a **finalise close-out** (aftercare · recall · 2-day wellbeing call · adverse event) before checkout (**REQ-CLIN-9**, ADR-0024). See [requirements §12](../02-requirements.md#12-prototype-alignment--feasibility-register).
>
